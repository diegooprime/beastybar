...
from . import actions, formatting, rules, state
...
def _resolve_skunk(game_state: state.State, card: state.Card, action: actions.Action) -> state.State:
    ...
    # Group by species (excluding skunks) and get their strength
    species_in_queue: dict[str, int] = {}
    for c in queue:
        if c.species != "skunk":
            species_in_queue[c.species] = c.strength

    if not species_in_queue:
        return game_state

    # Get top 2 species by strength
    sorted_species = sorted(species_in_queue.items(), key=lambda x: x[1], reverse=True)
    top_two_species = {species for species, _ in sorted_species[:2]}

    # Expel all animals of those species
    remaining: list[state.Card] = []
    for c in queue:
        if c.species == "skunk":
            remaining.append(c)
        elif c.species in top_two_species:
            game_state = state.push_to_zone(game_state, rules.ZONE_THATS_IT, c)
        else:
            remaining.append(c)
...
def _recurring_hippo(game_state: state.State, index: int) -> tuple[state.State, int, str]:
    ...
    if target == index:
        if index == 0:
            message = f"{formatting.card_label(hippo)} already leads the line."
        elif blocker is not None:
            message = f"{formatting.card_label(hippo)} is blocked by {formatting.card_label(blocker)}."
        else:
            message = f"{formatting.card_label(hippo)} holds position."
        return game_state, index + 1, message

    passed = queue[target:index]
    labels = ", ".join(formatting.card_label(card) for card in passed)
    message = (
        f"{formatting.card_label(hippo)} charges past {labels}."
        if labels
        else f"{formatting.card_label(hippo)} steps forward."
    )
...
def _recurring_crocodile(game_state: state.State, index: int) -> tuple[state.State, int, str]:
    ...
    while scan >= 0:
        ...
        scan -= 1
    if eaten:
        meal = ", ".join(formatting.card_label(card) for card in eaten)
        message = f"{formatting.card_label(crocodile)} eats {meal}."
        if blocker is not None:
            message += f" Stops before {formatting.card_label(blocker)}."
    else:
        if blocker is not None:
            message = f"{formatting.card_label(crocodile)} is blocked by {formatting.card_label(blocker)}."
        else:
            message = f"{formatting.card_label(crocodile)} already leads the line."
    return game_state, index + 1, message
...
def _recurring_giraffe(game_state: state.State, index: int) -> tuple[state.State, int, str]:
    ...
    if giraffe.entered_turn == game_state.turn:
        message = f"{formatting.card_label(giraffe)} waits until next turn."
        return game_state, index + 1, message
    if index == 0:
        message = f"{formatting.card_label(giraffe)} already leads the line."
        return game_state, index + 1, message
    ahead = queue[index - 1]
    if ahead.strength >= giraffe.strength:
        message = f"{formatting.card_label(giraffe)} is blocked by {formatting.card_label(ahead)}."
        return game_state, index + 1, message

    game_state, _ = state.remove_queue_card(game_state, index)
    game_state = state.insert_queue(game_state, index - 1, giraffe)
    message = f"{formatting.card_label(giraffe)} threads ahead of {formatting.card_label(ahead)}."

...
- The default batch-agent policy picks the first legal action. Tests should use custom agents (like the scripted helper in `_05_other/tests/test_simulate.py`) when deterministic behaviour is required.
...
- Update `_05_other/tests/test_replay.py` or add new fixtures when introducing card logic that alters turn sequencing so regression coverage stays accurate.
